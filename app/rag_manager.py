...
class RagAdminRepository:
    ...
    def get_next_product_id(self) -> int:
        ...
        try:
            cur.execute(
                """
                SELECT COALESCE(MAX(loan_product_id), 0) + 1
                FROM loan_product_documents
                """
            )
            row = cur.fetchone()
    ...
    def stream_ingest_pdf(
        ...
        *,
        ...
        def emit_log(message: str) -> None:
            ...
            queue.put(_serialize_event({"type": "log", "message": message}))

        def worker() -> None:
            try:
                emit_log("업로드 요청을 처리합니다.")

                result = self._ingest_pdf_impl(
                ...
                result["logs"] = logs
                queue.put(_serialize_event({"type": "result", "payload": result}))
            except ValueError as exc:
                queue.put(_serialize_event({"type": "error", "message": str(exc)}))
            except Exception as exc:
                print(f"stream_ingest_pdf error: {exc}")
                queue.put(
                    _serialize_event(
                        {"type": "error", "message": "문서 업로드 처리 중 오류가 발생했습니다."}
                    )
                )
            finally:
                queue.put(None)
